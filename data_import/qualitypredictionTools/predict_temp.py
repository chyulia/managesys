...
'''
def predict_temp(path_lf,heatno,path_kmeans,path_k,path_origin):
	...
	data2 = data.drop(['HEAT_NO','SPECIFICATION'],axis=1)
	data2 = data2.fillna(0)
	data2 = data2.ix[:,['TOTSENDPOWERTIME','C_1','TEMPOFARRIVE','CONSUMINGPOWER']]

	heatno = int(heatno)
	data_heat = data1[data1['HEAT_NO'] == heatno]
	data_ture = data_heat['TEMPOFTAPPING']
	data_heat = data_heat.drop(['HEAT_NO','SPECIFICATION'],axis=1)
	data_heat = data_heat.fillna(0)
	...
	print('222222222')
	print(int(data_ture),int(data_predict))
