...
def main(module,aspect,dateChoose,sql_date1,sql_date2,sql_cust,tradeNo,space,space_detail,module_unit_key):
	...
	dictionary = None #计算结果
	dictionary2 = None #地图识别的结果
	dictionary3 = {} #去除为零数据的结果
	dictionary4 = {} #包含全部结果的结果
	...
	if module == 1:  #空间分析	#前两个的 max_min_ave_sum 与 final_conclusion 是一样的
		...
		if space == 1:
			...
			nameDictionary = {'中国': 'China','马来西亚': 'Malaysia','韩国': 'South Korea',
				'英国':'United Kingdom','美国': 'United States of America','日本':'Japan',
				'泰国':'Thailand','沙特':'Saudi Arabia','新西兰':'New Zealand',
				'新加坡':'singpore','德国':'Germany','土耳其':'Turkey','墨西哥':'Mexico',
				'印度':'India','印尼':'Indonesia','其他':'others'}

			dictionary3["others"] = 0 
			for key in dictionary:
				
				# 中东 不在nameDictionary中
				if key in nameDictionary:  
					newKey = nameDictionary[key]
				else: 
					newKey = 'others'
				
				# 将非零值添加进去
				if key == 'others': #将不在nameDictionary列表中的值删除
					dictionary3[newKey] = dictionary3[newKey]  + dictionary[key]
				elif dictionary[key] != 0 and dictionary[key] != '总销量为0，无法计算退货率！':
					dictionary3[newKey] = dictionary[key]
				else:
					pass

			# 如果other的值为零，删除该值
			if dictionary3["others"] == 0:
				del dictionary3["others"]
			else:
				pass
			
			dictionary4["计算结果"] = dictionary
			dictionary4["地图结果"] = dictionary2
			dictionary4["饼图结果"] = dictionary3 #去零结果
			...
			for key in dictionary:
				newKey = key + "市"
				if dictionary[key] != 0 and dictionary[key] != '总销量为0，无法计算退货率！':
					dictionary3[newKey] = dictionary[key]
				else:
					pass
			dictionary4["计算结果"] = dictionary
			...
			for key in dictionary:
				if dictionary[key] != 0:
					...
					pass
			dictionary4["计算结果"] = dictionary
			dictionary4["地图结果"] = dictionary
			dictionary4["饼图结果"] = dictionary3 #去零结果
			#return dictionary,conclusionPrint,module_name,aspect_name,unite,maxValue

		return dictionary4,conclusionPrint,module_name,aspect_name,unite,maxValue
